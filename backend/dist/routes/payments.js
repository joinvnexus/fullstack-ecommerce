...
router.post("/stripe/webhook", async (req, res, next) => {
    try {
        ...
        const event = paymentService.verifyStripeSignature(JSON.stringify(req.body), signature);
...
router.post("/:orderId/refund", authenticate, authorizeAdmin, async (req, res, next) => {
    try {
        const { orderId } = req.params;
        ...
        const refund = await paymentService.createRefund(orderId, refundAmount);
        // Update order notes
        order.notes = `${order.notes || ""}\nRefund: ${reason} - Amount: $${amount || order.totals.grandTotal}`;
        await order.save();
        res.json({
            success: true,
...
router.post("/bkash/callback", async (req, res, next) => {
    try {
        ...
        if (callbackData.status === "success") {
            // Update order status
            await Order.findOneAndUpdate({ "payment.intentId": callbackData.paymentID }, {
                status: "processing",
...
router.post("/nagad/callback", async (req, res, next) => {
    try {
        ...
        if (callbackData.status === "Success") {
            await Order.findOneAndUpdate({ "payment.intentId": callbackData.paymentReferenceId }, {
                status: "processing",
                "payment.status": "succeeded",
                "payment.transactionId": callbackData.trxId,
                updatedAt: new Date(),
