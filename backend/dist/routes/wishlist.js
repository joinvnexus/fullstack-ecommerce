...
router.post('/move-item', async (req, res, next) => {
    try {
        ...
        const productItem = fromWishlist.items.find(item => item.productId.toString() === productId);
        // Remove from source
...
router.put('/:wishlistId', async (req, res, next) => {
    try {
        ...
        if (isDefault === true && !wishlist.isDefault) {
            await Wishlist.updateMany({ userId, _id: { $ne: wishlistId } }, { $set: { isDefault: false } });
