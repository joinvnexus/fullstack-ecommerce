...
export class BkashService {
    ...
    async getToken() {
        ...
        try {
            const response = await axios.post(`${this.config.baseURL}/checkout/token/grant`, {
    ...
    async createPayment(orderId, amount) {
        try {
            ...
            const response = await axios.post(`${this.config.baseURL}/checkout/payment/create`, paymentData, {
                headers: {
    ...
    async executePayment(paymentID) {
        try {
            const token = await this.getToken();
            const response = await axios.post(`${this.config.baseURL}/checkout/payment/execute/${paymentID}`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'X-APP-Key': this.config.appKey,
                    'Content-Type': 'application/json',
    ...
    async queryPayment(paymentID) {
        try {
            const token = await this.getToken();
            const response = await axios.get(`${this.config.baseURL}/checkout/payment/query/${paymentID}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'X-APP-Key': this.config.appKey,
                    'Content-Type': 'application/json',
