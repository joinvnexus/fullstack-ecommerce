...
export class NagadService {
    ...
    async initializePayment(orderId, amount) {
        try {
            ...
            }, Buffer.from(sensitiveDataString)).toString('base64');
            // Generate signature
            ...
            const response = await axios.post(`${this.config.baseURL}/api/dfs/check-out/initialize/${merchantID}/${invoiceNo}`, requestData, {
                headers: {
    ...
    async verifyPayment(paymentRefId) {
        try {
            ...
            const signature = this.generateSignature(signatureData);
            const response = await axios.post(`${this.config.baseURL}/api/dfs/verify/payment/${merchantID}/${paymentRefId}`, {
                merchantId: merchantID,
