...
router.get('/', async (req, res, next) => {
    try {
        const { page = 1, limit = 20, category, minPrice, maxPrice, status = 'active', search, sort = 'createdAt', order = 'desc', } = req.query;
        // Build query
        const query = {};
...
router.post('/', authenticate, authorizeAdmin, validate(createProductSchema), async (req, res, next) => {
    try {
...
router.delete('/:id', authenticate, authorizeAdmin, async (req, res, next) => {
    try {
        ...
        const product = await Product.findByIdAndUpdate(productId, { status: 'archived' }, { new: true });
        if (!product) {
            throw new AppError('Product not found', 404);
        }
        res.json({
